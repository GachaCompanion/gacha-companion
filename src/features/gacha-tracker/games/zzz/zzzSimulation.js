// ZZZ character banner: base 0.6%, soft pity +6%/pull from pull 74, hard pity 90.
function getCharRate(pity) {
  return Math.min(1.0, 0.006 + 0.06 * Math.max(0, pity - 73));
}

// ZZZ W-Engine banner: base 0.8%, soft pity +7%/pull from pull 63, hard pity 80.
function getWERate(pity) {
  return Math.min(1.0, 0.008 + 0.07 * Math.max(0, pity - 62));
}

const CHAR_HARD_PITY = 90;
const WE_HARD_PITY = 80;

// Standard advertised rates — no research adjustment applied.
const CHAR_FEATURED_RATE = 0.5;
const WE_FEATURED_RATE = 0.75;
...
export function computeZZZ({
  charCopies = 0,
  weCopies = 0,
  startCharPity = 0,
  startCharGuaranteed = false,
  startWEPity = 0,
  startWEGuaranteed = false,
}) {
  ...
  if (weCopies > 0) {
    const freshWEPmf = buildCopyPmf(0, false, WE_FEATURED_RATE, WE_HARD_PITY, getWERate);
    let cumPmf = buildCopyPmf(startWEPity, startWEGuaranteed, WE_FEATURED_RATE, WE_HARD_PITY, getWERate);
    weMilestones.push({ pmf: cumPmf, cdf: buildCdf(cumPmf) });
    for (let w = 1; w < weCopies; w++) {
      cumPmf = convolvePmf(cumPmf, freshWEPmf);
      weMilestones.push({ pmf: cumPmf, cdf: buildCdf(cumPmf) });
    }
  }

  return { charMilestones, weMilestones };
}
