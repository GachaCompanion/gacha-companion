...
// WuWa's Radiant Tide (character) and Forging Tide (weapon) aren't
// interchangeable, so like NteStatusTab this tracks them as two separate
// pull-item counts against the same Astrite currency.
export default function WuwaStatusTab({ game, onUpdate, color }) {
  const t = useT();
  const { state, charBanner } = game;
  const costPerPull = charBanner.costPerPull;
  const { currencyName, pullItemName: charTideLabel, weaponPullItemName: weaponTideLabel } = resolveGameCurrency(game);
  ...
  // reads) tracks currency changes as 'income' and Tide-item changes as
  // 'pulls' (both Tide types share the one 'pulls' column — the ledger UI
  ...
  // (currency + pullItems*costPerPull), WuWa's two separate pull-item pools
  ...
  // beyond the currency itself (matching what WuWa's own Status tab already
  ...
  const rLabel = charTideLabel || t('Character Pull Items');
  const fLabel = weaponTideLabel || t('Weapon Pull Items');
      ...
      <div className="status-section">
        ...
        <div className="resources-grid resources-grid--triple">
          ...
          <ResourceCard
            ...
            sub={`${weaponTotalPulls} ${t('weapon pulls')}`}
            color={color}
        ...
        <div className="total-bar">
          ...
          <div className="total-bar-item total-bar-item--right">
            <span className="total-bar-label">{t('Weapon pulls')}</span>
